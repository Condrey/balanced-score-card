...
export const performanceObjectiveSchema = z
	.object({
		id: z.string().optional().describe("this should be a random uuid() number"),
		perspective: z.enum(["STAKEHOLDERS_CLIENTS", "FINANCIAL_STEWARDSHIP", "INTERNAL_PROCESSES", "MDA_LG_CAPACITY"])
      ...
      4. MDA_LG_CAPACITY: Focuses on the potential and the ability of the MDA/LG to deliver on its mandate, score or 20%`),
		...
		kpis: z.array(stringArraySchema).min(1, "At least one KPI is required").describe(`
      ...
      `),
		score: z.number().min(0).max(100).describe("Actual performance score achieved"),
...
export const bscSchema = z.object({
	id: z.string().optional().describe("this should be a random uuid() number"),
	organizationId: z.string().optional().describe("this should be a random uuid() number"),
...
export type PerformanceObjectiveArraySchema = z.infer<typeof performanceObjectiveArraySchema>;
export type CoreValuesSchema = z.infer<typeof coreValuesSchema>;
export type BehavioralAttributeSchema = z.infer<typeof behavioralAttributeSchema>;
