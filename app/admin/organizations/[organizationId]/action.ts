...
import { ndpDataInclude, organizationContextDataInclude, organizationDataInclude, ospDataInclude } from "@/lib/types";
import {
	NdpSchema,
	ndpSchema,
...
export async function upsertOrganizationContext(input: OrganizationContextSchema) {
	// TODO: perform auth
	const { id, financialYear, mandate, vision, mission, goal, organizationId } = organizationContextSchema.parse(input);
...
export async function upsertNdp({ input, organizationContextId }: { input: NdpSchema; organizationContextId: string }) {
	// TODO: perform auth
...
export async function upsertOsp(input: OspSchema) {
	// TODO: perform auth
	const { id, ndpId, programmes, strategicObjective, strategies } = ospSchema.parse(input);
	const formattedProgrammes = programmes.map((p) => p.value).filter(Boolean) as string[];
	const formattedStrategies = strategies.map((p) => p.value).filter(Boolean) as string[];
