...
export default function ButtonDeleteOrganizationContext({
	...
	return (
		<>
			<Button
				title={organizationContext ? "Edit OrganizationContext" : "Add OrganizationContext"}
				onClick={() => setOpen(true)}
				{...props}
			/>
			<DialogDeleteOrganizationContext open={open} setOpen={setOpen} organizationContext={organizationContext} />
		</>
	);
}
...
export function DialogDeleteOrganizationContext({
	...
	const deleteItem = () => mutate(organizationContext.id, { onSuccess: () => setOpen(false) });
