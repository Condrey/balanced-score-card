...
import { OspSchema, stringArraySchema, StringArraySchema } from "@/lib/validations/others";
		...
		<>
			<FormField
				...
				render={() => (
					<FormItem>
						<FormLabel>
							Programmes <span className="text-xs text-muted-foreground">({form.watch("programmes")?.length})</span>
						</FormLabel>
						<FormField
			...
			<ol className="flex list-inside list-decimal flex-col gap-2">
				{fields.map((field, index) => (
					<li key={field.id} className={cn("w-fit max-w-sm gap-1", badgeVariants({ variant: "secondary" }))}>
						<span className="line-clamp-1 text-ellipsis">{form.watch(`programmes.${index}.value`)}</span>
						<Button
							type="button"
							variant="ghost"
							size="sm"
