		...
		<>
			<FormField
				...
				render={() => (
					<FormItem>
						<FormLabel>
							Strategies <span className="text-xs text-muted-foreground">({form.watch("strategies")?.length})</span>
						</FormLabel>
						<FormField
			...
			<ol className="flex list-inside list-decimal flex-col gap-2">
				{fields.map((field, index) => (
					<li key={field.id} className={cn("w-fit max-w-sm gap-1", badgeVariants({ variant: "secondary" }))}>
						<span className="line-clamp-1 text-ellipsis">{form.watch(`strategies.${index}.value`)}</span>
						<Button
							type="button"
							variant="ghost"
							size="sm"
