...
export default function ListOfBehavioralAttributes({
	...
	return (
		<Table>
			...
			<TableBody>
				{behavioralAttributes.map((behavioralAttribute, index) => (
					<TableRow key={behavioralAttribute.id}>
						...
						<TableCell className={cn(!behavioralAttribute.description && "italic text-muted-foreground")}>
							{behavioralAttribute.description || "--Please edit to add description.--"}
						</TableCell>
						<TableCell>
			...
			<TableCaption>This table shows the behavioral attributes as extracted from the core values.</TableCaption>
		</Table>
	);
}
