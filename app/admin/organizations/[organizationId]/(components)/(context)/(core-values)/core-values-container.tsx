...
export default function CoreValuesContainer({ coreValue, organizationContext }: CoreValuesContainerProps) {
	return (
		<Card>
			<CardHeader className="flex flex-row items-center justify-between gap-2">
				...
				<div className="flex gap-2">
					<ButtonAddEditCoreValue size={"icon"} coreValue={coreValue} context={organizationContext}>
						<Edit3Icon />
					</ButtonAddEditCoreValue>
					<ButtonDeleteCoreValue variant={"destructive"} size={"icon"} coreValue={coreValue}>
						<Trash2Icon />
			...
			<CardContent>
				<p className="max-w-3xl *:inline">
					Our value system for FY{organizationContext.financialYear} that will shape our culture is{" "}
					<q className="uppercase">
						<strong>{coreValue.acronym}</strong>
					</q>{" "}
					for {coreValue.values.map((v) => `${v.value} (${v.score}%)`).join(", ")}.
				</p>
			</CardContent>
			...
			<CardHeader>
				...
				<CardDescription>Assessments curved from the the core values of the organization.</CardDescription>
			</CardHeader>
			<CardContent>
				{!organizationContext.behavioralAttributes.length ? (
					<EmptyContainer
						message={"It is high time you updated the Behavioral attributes since you have the core values."}
