...
export function upsertOrganizationContextMutation() {
	...
	return useMutation({
		...
		async onSuccess(data, variables, context) {
			...
			toast.success(`Successfully ${!variables.id ? "created" : "updated"}  the ${variables.financialYear} context`);
		},
...
export function deleteOrganizationContextMutation() {
	...
	return useMutation({
		...
		async onSuccess(data, variables, context) {
			...
			await queryClient.cancelQueries({ queryKey });
			queryClient.setQueryData<OrganizationContextData[]>(queryKey, (oldData) => {
				if (!oldData) return;
				toast.success(`${data.financialYear} was successfully removed from the database.`);
...
export function deleteNdpMutation(organizationId: string) {
	...
	return useMutation({
		...
		async onSuccess(data, variables, context) {
			...
			queryClient.setQueryData<OrganizationContextData[]>(queryKey, (oldData) => {
				if (!oldData) return;
				toast.success(`NDP ${data.version} version was successfully removed from the database.`);
				return oldData.filter((d) => d.id !== data.id);
			});
		},
		onError: (error, variables, context) => {
...
export function upsertOspMutation() {
	...
	return useMutation({
		...
		async onSuccess(data, variables, context) {
			...
			await queryClient.cancelQueries({ queryKey });
			queryClient.invalidateQueries({ queryKey });
			toast.success(`Successfully ${!variables.id ? "created" : "updated"}  the OSP `);
		},
		onError: (error, variables, context) => {
			console.error(error);
