...
export default function ButtonDeleteOrganization({ organization, ...props }: ButtonDeleteOrganizationProps) {
...
export function DialogDeleteOrganization({ open, setOpen, organization }: DialogDeleteOrganizationProps) {
	const { isPending, mutate } = deleteOrganizationMutation();
	const deleteItem = () => mutate(organization.id, { onSuccess: () => setOpen(false) });
