...
export const usePositionColumns: ColumnDef<PositionData>[] = [
		...
		cell({ row }) {
			...
			return (
				<div>
					...
					<div className="text-xs text-muted-foreground">{staff.salaryScale}</div>
				</div>
			);
		}
	},
	...
	},
	{
		...
		header({ column }) {
			return <DataTableColumnHeader column={column} title="Departmental Mandate" />;
		},
		cell({ row }) {
			return <p className="line-clamp-2 text-ellipsis text-sm">{row.original.departmentalMandate}</p>;
		}
	},
	{
