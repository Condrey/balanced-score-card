...
export default function FormSupervisor({ form }: FormSupervisorProps) {
	...
	if (status === "error")
		return <ErrorContainer errorMessage="Failed to fetch supervisors" query={query} className="min-h-fit" />;
	if (status === "success" && !data.length)
		return <EmptyContainer message="No supervisors found. Please add positions first" className="min-h-fit" />;
	return (
		<FormField
					...
					<Popover>
						<PopoverTrigger asChild>
							<FormControl>
								...
								>
									{field.value ? data.find((d) => d.id === field.value)?.jobTitle : "Select supervisor"}
								...
								<CommandList>
									...
									<CommandGroup>
										...
										{data.map((d) => (
											...
											>
												{d.jobTitle}
												<Check className={cn("ml-auto", d.id === field.value ? "opacity-100" : "opacity-0")} />
											</CommandItem>
										))}
									</CommandGroup>
								</CommandList>
