...
export default function FormAddEditPosition({ position, open, setOpen }: FormAddEditPositionProps) {
	...
	return (
		<ResponsiveDrawer
			...
			description={position ? "Make changes to this position" : "Create a new position in the database."}
			className="max-w-4xl"
		>
			<Form {...form}>
				<form
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Textarea placeholder="job purpose from Job description " {...field} />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
