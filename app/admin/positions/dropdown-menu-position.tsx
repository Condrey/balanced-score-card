...
export default function DropdownMenuPosition({ position }: DropdownMenuPositionProps) {
	const [openEdit, setOpenEdit] = useState(false);
	const [openDelete, setOpenDelete] = useState(false);
	const [isPending, startTransition] = useTransition();
	const { getNavigationLinkWithPathnameWithoutUpdate } = useCustomSearchParams();
	const newUrl = getNavigationLinkWithPathnameWithoutUpdate(`/admin/positions/${position.id}`);
	return (
		<>
			...
			<FormAddEditPosition open={openEdit} setOpen={setOpenEdit} position={position} />
			<DialogDeletePosition open={openDelete} setOpen={setOpenDelete} position={position} />
		</>
	);
}
