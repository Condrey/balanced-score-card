...
export default function PositionContainer({ initialData }: PositionContainerProps) {
	...
	return (
		<div className="flex w-full flex-col gap-4">
			{status === "error" ? (
				<ErrorContainer errorMessage="Failed to fetch position details" query={query} />
				...
				<div className="mx-auto flex w-full max-w-7xl flex-wrap gap-3">
					<Card className="max-w-lg">
						<CardHeader className="flex flex-row justify-between gap-2">
							<div>
								<CardTitle>
									{" "}
									<span className="text-muted-foreground">JOB TITLE :</span> {position?.jobTitle}
								</CardTitle>
								<CardTitle>
									<span className="text-muted-foreground">SALARY SCALE :</span> {position?.salaryScale}
								</CardTitle>
							</div>
						...
						<CardHeader>
							{" "}
							<CardTitle className=" ">Job Purpose/ Departmental Mandate</CardTitle>
							<CardDescription className="hyphens-auto text-justify">{position?.departmentalMandate}</CardDescription>
						</CardHeader>
						<CardContent>
					...
					<div className="space-y-6">
						<Card className="text:green-50 max-w-lg bg-green-500 dark:bg-green-300/20">
							<CardHeader>
								<CardTitle className="uppercase text-muted-foreground">Reports to</CardTitle>
							</CardHeader>
							<CardContent>
								{position?.reportsTo ? (
									<>
										...
										<CardDescription>{position?.reportsTo?.salaryScale}</CardDescription>
									</>
								) : (
						...
						<Card className="text:red-50 max-w-lg bg-red-500 dark:bg-red-300/20">
							<CardHeader>
								<CardTitle className="uppercase text-muted-foreground">Responsible for</CardTitle>
							</CardHeader>
							<CardContent>
