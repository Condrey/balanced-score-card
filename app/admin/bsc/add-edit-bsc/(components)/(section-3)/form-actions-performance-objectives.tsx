...
export default function FormActionsPerformanceObjectives({ form }: FormActionsPerformanceObjectivesProps) {
			...
			<FormField
				...
				render={() => (
					<FormItem className={cn(!!watchedValues.length && "bg-muted p-1")}>
						<FormLabel>
							Actions/Activities <span className="text-xs text-muted-foreground">({watchedValues?.length})</span>
						</FormLabel>
						<FormField
