...
import { BSCFormData, performanceObjectiveSchema, PerformanceObjectiveSchema } from "@/lib/validations/bsc";
...
export default function FormPerformanceObjectives({ form, isPending, isError }: FormPerformanceObjectivesProps) {
				...
				render={() => (
					<FormItem>
						<div className="space-y-4">
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<NumberInput placeholder="enter percentage" min="0" max="100" step="0.1" {...field} />
											</FormControl>
											<FormMessage />
										</FormItem>
									)}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											<Textarea rows={2} placeholder="enter performance objective" {...field} />
										</FormControl>
										<FormMessage />
									</FormItem>
								)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<NumberInput min="0" max="100" placeholder="Enter score" {...field} />
											</FormControl>
											<FormMessage />
										</FormItem>
									)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Textarea placeholder="Comments on actual performance" rows={2} {...field} />
											</FormControl>
											<FormMessage />
										</FormItem>
									)}
							...
							<Button type="button" onClick={() => form2.handleSubmit(addValue)()} className="w-full">
								<PlusIcon className="mr-2 h-4 w-4" />
								Add Performance Objective
							</Button>
							<Card className="bg-sidebar">
								<CardHeader>
									<CardDescription>{watchedValues.length} objective(s) available</CardDescription>
								</CardHeader>
								<CardFooter className="flex flex-wrap gap-2">
									{watchedValues.map((wv, index) => (
										<HoverCard key={index}>
											<HoverCardTrigger className={cn("text-sm", buttonVariants({ size: "sm", variant: "secondary" }))}>
											...
											<HoverCardContent>
												...
												{[
													...
													},
													{
														...
														content: wv.actions.map((a) => `- ${a.value}`).join(",\n")
													},
													{
														title: "Expected results",
														content: wv.expectedResults.map((a) => `- ${a.value}`).join(",\n")
													},
													{
														title: "Key Performance Indicators",
														content: wv.kpis.map((a) => `- ${a.value}`).join(",\n")
													}
												].map((obj, index) => (
													<div key={index}>
														<span className="text-sm font-semibold">{obj.title}</span>
														<p className="whitespace-pre-line text-xs text-muted-foreground">{obj.content}</p>
													</div>
												))}
											</HoverCardContent>
										</HoverCard>
						...
						{isPending && <FormDescription>Ai is helping with your performance objectives...</FormDescription>}
						<FormMessage>
							{isError &&
