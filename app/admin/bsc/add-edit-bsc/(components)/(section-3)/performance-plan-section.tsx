...
import type { BSCFormData, PerformanceObjectiveArraySchema } from "@/lib/validations/bsc";
		...
		<div className="space-y-6">
					...
					<div className="grid grid-cols-2 gap-1 md:grid-cols-4">
						{Object.entries(PERSPECTIVE_ALLOCATIONS).map(([key, { label, percentage }]) => {
							...
							return (
								...
								>
									<div className="line-clamp-2 text-ellipsis text-sm font-medium">{label}</div>
									<div
			...
			<Card>
				<CardHeader className="mb-2 flex flex-row items-center justify-between gap-2 bg-secondary">
					...
					>
						<StarsIcon className="" />
						<span className="sr-only">Use Ai to create your performance objectives</span>
					</LoadingButton>
				</CardHeader>
				<CardContent>
					<FormPerformanceObjectives form={form} isError={isError} isPending={isPending} />
				</CardContent>
			</Card>
		</div>
	);
