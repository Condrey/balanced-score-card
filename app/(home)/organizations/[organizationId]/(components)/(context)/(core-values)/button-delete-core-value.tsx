...
export default function ButtonDeleteCoreValue({
  coreValue,
  ...props
}: ButtonDeleteCoreValueProps) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <Button
        title={coreValue ? "Edit CoreValue" : "Add CoreValue"}
        onClick={() => setOpen(true)}
        {...props}
      />
      <DialogDeleteCoreValue
        open={open}
        setOpen={setOpen}
        coreValue={coreValue}
      />
...
export function DialogDeleteCoreValue({
  open,
  setOpen,
  coreValue,
}: DialogDeleteCoreValueProps) {
  const { isPending, mutate } = deleteCoreValuesMutation();
  const deleteItem = () =>
    mutate(coreValue.id, { onSuccess: () => setOpen(false) });
