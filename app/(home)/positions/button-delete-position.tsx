...
export default function ButtonDeletePosition({
  position,
  ...props
}: ButtonDeletePositionProps) {
...
export function DialogDeletePosition({
  open,
  setOpen,
  position,
}: DialogDeletePositionProps) {
  const { isPending, mutate } = deletePositionMutation();
  const deleteItem = () =>
    mutate(position.id, { onSuccess: () => setOpen(false) });
